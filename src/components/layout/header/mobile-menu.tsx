...
export default function MobileMenu() {
	...
	return (
		<>
			<div className="flex flex-col justify-between bg-black text-white w-full h-full">
				...
				<div className="flex items-center justify-center bg-black border-t border-gray-100 px-7 flex-shrink-0 space-s-1">
					{social?.map((item, index) => (
						<a
							href={item.link}
							className={`text-heading  text-white p-5 opacity-60 first:-ms-4 transition duration-300 ease-in hover:opacity-100 ${item.className}`}
							target="_blank"
							key={index}
						>
