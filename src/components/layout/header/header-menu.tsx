...
const HeaderMenu: React.FC<MenuProps> = ({ data, className }) => {
	...
	return (
		<nav className={classNames(`headerMenu flex text-white w-full relative`, className)}>
			{data?.map((item: any) => (
				<div
				...
				>
					...
					>
						...
						{(item?.columns || item.subMenu) && (
							<span className="opacity-30 text-xs  text-white mt-1 xl:mt-0.5 w-4 flex justify-end">
