...
const ContactInfoBlock: FC<Props> = () => {
	...
	return (
		<div className="mb-6 lg:border lg:rounded-md border-gray-300 lg:p-7" >
			<h4 className="text-2xl text-white md:text-lg font-bold text-heading pb-7 md:pb-10 lg:pb-6 -mt-1">
				{t("text-find-us-here")}
			</h4>
			{data?.map((item: any) => (
				<div key={`contact--key${item.id}`} className="flex pb-7">
					...
					<div className="flex flex-col ps-3 2xl:ps-4">
						<h5 className="text-sm font-bold text-white text-heading">
							{t(`${item.name}`)}
						</h5>
						<Link href={item.slug} className="text-sm text-white  mt-0">
							{t(`${item.description}`)}
						</Link>
					</div>
				</div>
